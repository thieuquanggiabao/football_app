...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  late Future<Map<String, dynamic>?> _profileFuture = _profileService
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '🎉 Đã thiết lập ${team.teamName} làm đội bóng yêu thích!',
            ),
          ),
        );
      }
  ...
  void _showTeamSelection() {
    showModalBottomSheet(
      ...
      builder: (_) => TeamSelectionSheet(onTeamSelected: _updateFavoriteTeam),
    );
  }

  Widget _buildVipBadge(Map<String, dynamic>? profile, BuildContext context) {
    if (profile == null ||
        profile['is_premium'] != true ||
        profile['plan_code'] == null) {
      return Container(
    ...
    if (profile['expire_date'] != null) {
      try {
        final expireDate = DateTime.parse(profile['expire_date']).toLocal();
        expireText =
            '\nHết hạn: ${expireDate.day}/${expireDate.month}/${expireDate.year}';
      } catch (e) {
        // ignore parse error
      }
    }
      ...
      body: FutureBuilder<Map<String, dynamic>?>(
        ...
        builder: (context, snapshot) {
          ...
          final userName =
              ...
              'Fan Bóng Đá';
          final avatarUrl =
              profile?['avatar_url'] ?? user?.userMetadata?['avatar_url'] ?? '';
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                _buildVipBadge(profile, context),
